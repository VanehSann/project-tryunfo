...
onDeleteClick = (item) => {
  ...
  const items = getItem(cardArray, item.key) // Method to get item in list through comparison (IE: find some item with item.id), it has to return ITEM and INDEX in array
  const newlist = [].concat(cardArray) // Clone array with concat or slice(0)
  newlist.splice(items.index, 1);
